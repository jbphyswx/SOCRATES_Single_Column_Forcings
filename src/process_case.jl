...
function process_case(
    ...
    if ~isnothing(surface) #(is always ERA5) -- maybe not, doesn't seem so in ATLAS LES outputs
        ...
        elseif surface ∈ ["surface_conditions", "conditions", "cond"]
            ...
            return (;
                pg = t -> pyinterp([t], tg, pg)[1],
                Tg = t -> pyinterp([t], tg, Tg)[1],
                qg = t -> pyinterp([t], tg, qg)[1],
    ...
    ground_indices =
        map((ts) -> get_ground_insertion_indices(ts, tsg[forcing], z_dim_num; thermo_params, data = data), ts) # always use ERA5 surface...
