...
if setup_environment
    ...
    namelist["microphysics"]["τ_cond_evap"] = 1.
...
truth_file = joinpath(SOCRATES_truth_path, "RF"*string(flight_number, pad=2)*"_"*forcing_str*"_SOCRATES_128x128_100m_10s_rad10_vg_M2005_aj.nc")
...
p_t = plot(data_truth, z_truth, label="Truth", legend=:bottomleft,
    xlabel="T (K)", ylabel="Height (m)", title="Temperature (K)", ylim=(ymin,ymax),
    )
p_s = plot!( data_simul, z_simul, label="Simulation",) # add simulation data to same plot
# vertical line at 5.22 + 273.15 degrees
# surface temp dot 
plot!(simul_data.group["timeseries"]["Tsurface"][[simul_ind]], [0], label="Surface Temp", seriestype=:scatter, markersize=5, markercolor=:red)
...
p_t = plot(data_simul .- pyinterp(z_simul, z_truth, data_truth,  bc="extrapolate") , z_simul, label="Truth", legend=:bottomright, ylim=(ymin,ymax),
    xlabel="T diff (K)", ylabel="Height (m)", title="Temperature (K) diff",
    )
...
p_t = plot(data_truth, z_truth, label="Truth", legend=:bottomright, ylim=(ymin,ymax),
    xlabel="qt (kg/kg)", ylabel="Height (m)", title="QT (kg/kg)",
    )
p_s = plot!( data_simul, z_simul, label="Simulation",) # add simulation data to same plot
savefig(joinpath(outpath, "Figures", "qt.png")) # save to file

p_t = plot(data_simul .- pyinterp(z_simul, z_truth, data_truth, bc="extrapolate"), z_simul, label="diff", legend=:bottomright, ylim=(ymin,ymax),
    xlabel="qt diff (K)", ylabel="Height (m)", title="qt_diff",
    )
...
p_t = plot(data_truth, z_truth, label="Truth", legend=:bottomright, ylim=(ymin,ymax),
    xlabel="ql (kg/kg)", ylabel="Height (m)", title="QL (kg/kg)",
    )
p_s = plot!( data_simul, z_simul, label="Simulation",) # add simulation data to same plot
savefig(joinpath(outpath, "Figures", "ql.png")) # save to file

p_t = plot(data_simul .- pyinterp(z_simul, z_truth, data_truth,  bc="extrapolate"), z_simul, label="diff", legend=:bottomright, ylim=(ymin,ymax),
    xlabel="ql diff (K)", ylabel="Height (m)", title="ql_diff",
    )
...
p_t = plot(data_truth, z_truth, label="Truth", legend=:bottomright, ylim=(ymin,ymax),
    xlabel="qi (kg/kg)", ylabel="Height (m)", title="QI (kg/kg)",
    )
p_s = plot!( data_simul, z_simul, label="Simulation",) # add simulation data to same plot
savefig(joinpath(outpath, "Figures", "qi.png")) # save to file

p_t = plot(data_simul .- pyinterp(z_simul, z_truth, data_truth;  bc="extrapolate"), z_simul, label="diff", legend=:bottomright, ylim=(ymin,ymax),
    xlabel="qi diff (K)", ylabel="Height (m)", title="qi_diff",
    )
...
# ql_ind = 1
data_simul = simul_data.group["profiles"]["ql_mean"][:,qc_simul_ind] + simul_data.group["profiles"]["qi_mean"][:,qc_simul_ind] 
data_truth = truth_data["QCL"][:,qc_truth_ind] ./ 1000 + truth_data["QCI"][:,qc_truth_ind] ./ 1000

p_t = plot(data_truth, z_truth, label="Truth", legend=:bottomright, ylim=(ymin,ymax),
    xlabel="qc (kg/kg)", ylabel="Height (m)", title="QC (kg/kg)",
    )
p_s = plot!( data_simul, z_simul, label="Simulation",) # add simulation data to same plot
savefig(joinpath(outpath, "Figures", "qc.png")) # save to file

p_t = plot(data_simul .- pyinterp(z_simul, z_truth, data_truth;  bc="extrapolate"), z_simul, label="diff", legend=:bottomright, ylim=(ymin,ymax),
    xlabel="qc diff (K)", ylabel="Height (m)", title="qc_diff",
    )
...
p_t = plot(data_truth, z_truth, label="Truth", legend=:bottomright, ylim=(ymin,ymax),
    xlabel="theta_l (K)", ylabel="Height (m)", title="theta_l (K)",
    )
p_s = plot!( data_simul, z_simul, label="Simulation",) # add simulation data to same plot
savefig(joinpath(outpath, "Figures", "theta_l.png")) # save to file

p_t = plot(data_simul .- pyinterp(z_simul, z_truth, data_truth;  bc="extrapolate"), z_simul, label="diff", legend=:bottomright, ylim=(ymin,ymax),
    xlabel="theta_l diff (K)", ylabel="Height (m)", title="thetal_diff",
    )
...
p_t = plot(data_truth_ql, z_truth, label="Truth liq", legend=:bottomright, ylim=(ymin,ymax), color=:green,
    xlabel="q (kg/kg)", ylabel="Height (m)", title="QL and QI (kg/kg)",
    )
...
plot(data_simul_ql_up, z_simul, label="Updraft liq", legend=:bottomright, ylim=(ymin,ymax), color=:green,
    xlabel="q (kg/kg)", ylabel="Height (m)", title="QL and QI (kg/kg)",
    )
...
p_t = plot(data_simul_up, z_simul, label="Updraft", legend=:bottomleft, ylim=(ymin,ymax), color=:red,
    xlabel="T (K)", ylabel="Height (m)", title="Updraft and Environmental Temperature (K)",
    )
...
data_truth = truth_data["QT"][:,truth_ind] ./ 1000 + truth_data["QS"][:,truth_ind] ./ 1000 + truth_data["QR"][:,truth_ind] ./ 1000
...
p_t = plot(data_truth, z_truth, label="Truth", legend=:bottomleft, ylim=(ymin,ymax), dpi=600, marker=:circle, markersize=1.5,
    xlabel="qt (kg/kg)", ylabel="Height (m)", title="QT (kg/kg)",
    )
p_s = plot!( data_simul, z_simul, label="Simulation",) # add simulation data to same plot
...
p_t = plot(data_truth_start, p_truth_start, label="Truth", legend=:bottomleft, dpi=600, marker=:circle, markersize=1.5, yflip=true,
    xlabel="qt (kg/kg)", ylabel="Height (m)", title="QT (kg/kg)",
    )
...
plot(data_simul, z_simul, label="Updraft Area", legend=:topright, ylim=(ymin,ymax),
    xlabel="Area", ylabel="Height (m)", title="Updraft Area",
    )
